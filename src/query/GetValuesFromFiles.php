...
class GetValuesFromFiles
{
    ...
    protected function loadFileValues(array $rows, ?bool $public = true): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $result[$key] = ($this->castTypeHandler)($value);

            if ($public) {
                $this->publish($key, $value, $result[$key]);
            }
        }
     ...
     */
    protected function publish(string $key, string $value, $castValue): void
    {
